...
export const useCollection = (collectionName, orderField = null, orderDir = 'desc') => {
  ...
  const addData = useCallback(async (payload) => {
    if (!user) return null;
    try {
...
export const useUserDoc = (collectionName, defaultData = {}) => {
  ...
  const updateDoc_ = useCallback(async (payload) => {
    if (!user) return { success: false, error: 'No user authenticated' };
    const docRef = doc(db, collectionName, user.uid);
    try {
      await setDoc(docRef, { ...payload, userId: user.uid, updatedAt: new Date().toISOString() }, { merge: true });
      return { success: true };
    } catch (err) {
