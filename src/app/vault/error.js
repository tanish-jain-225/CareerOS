  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center min-h-[70vh] p-6 space-y-8 animate-in zoom-in-95 duration-500">
      <div className="relative">
        ...
        <div className="p-8 rounded-[2.5rem] bg-white/[0.02] border border-rose-500/20 relative z-10 shadow-2xl overflow-hidden">
      ...
      <div className="text-center space-y-4 max-w-md">
        <div className="flex items-center justify-center gap-3">
          ...
          <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/40">Sector Breach</h2>
        </div>
        <h1 className="text-2xl font-black font-outfit text-white leading-tight">Module Desynchronization</h1>
        <p className="text-xs font-medium text-white/20 leading-relaxed">
          The tactical node encountered an unhandled exception. This could be due to a network interruption or a corrupted data stream.
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <Link
          href="/"
          className="btn-secondary h-14 px-8 flex items-center justify-center gap-3 active:scale-95 transition-all"
        >
      ...
      {process.env.NODE_ENV === 'development' && (
        <div className="panel bg-rose-500/5 border-rose-500/10 p-4 rounded-xl mt-8">
          <p className="text-[10px] font-mono text-rose-400 break-all">{error?.message || 'Unknown Tactical Error'}</p>
        </div>
      )}
    </div>
  );
